...
TensorCash Web Wallet v1.0.8 is a non-custodial, browser-first wallet for TensorCash. It creates locally encrypted, descriptor-based TensorCash Core/Qt `wallet.dat` wallets and can import supported Qt backups without uploading the file. Compatible browsers can use a device Passkey first while retaining the wallet password as a permanent fallback. End users do not need to run a full node; keys and transaction signing stay in their browser.
...
- Supports optional, per-wallet Passkey authorization without changing the existing password-encrypted vault format or removing password fallback.
...
The current patch release is **v1.0.8**. Keep root `package.json`, `web/package.json`, `package-lock.json`, the release tag, and release notes aligned. Document any required Core version, gateway API change, backup migration, or operator action.
