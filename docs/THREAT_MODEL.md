...
| API denial of service | Bounded address/page/body/UTXO/mempool sizes, 50-call RPC chunks, in-app per-IP token bucket, short public-read timeouts, stale-safe chain/mempool caches and Nginx limit | Distributed resource exhaustion remains possible; stale metadata must not be treated as authorization to spend |
